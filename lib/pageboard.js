...
const resolvePkg = require.lazy('resolve-pkg');
...
module.exports = class Pageboard {
	...
	static async create(cliOpts = {}) {
		...
		opts.config = cliOpts.config ?? Path.join(opts.dirs.config, 'config');

		const fileOpts = toml.parse(await fs.readFile(opts.config));

		mergeRecursive(opts, fileOpts, cliOpts);
		...
		await app.init();
		if (opts.server.start) await app.start();
		return app;
	}
	...
	constructor(opts) {
		...
		opts = mergeRecursive(defaults, opts);
		...
		this.#loadPlugins({
			plugins: (await Promise.all(
				this.opts.plugins.map(module => {
					const pkgPath = resolvePkg(module, { cwd: this.dirs.app });
