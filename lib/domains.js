...
module.exports = class Domains {
	...
	async byHost(req, res, next) {
		this.initTenant(req);
		const { path } = req;
		const host = await this.init(req);
		...
		if (path == "/.well-known/upcache") {
			...
			await p;
			if (host.isWaiting && !req.path.startsWith('/.') && this.app.env != "development") {
				next = null;
		...
		if (tenant) {
			if (!host.tenants[tenant]) {
				const tsite = await this.app.run('site.get', req, { id: host.id });
			...
			if (domains.length && req.hostname != domains[0] && !req.path.startsWith('/.')) {
				...
				await rhost.waiting;
				this.app.cache.tag('data-:site')(req, res, () => {
		...
		if (path == "/.well-known/status" || path == "/.well-known/pageboard") {
			...
			if (version != this.app.version) {
				res.set('X-Pageboard-Peer', this.app.upstreams[version]);
	...
	async syncSites(req, res) {
		const list = await this.app.run('site.all', req);
		const map = {};
		const siteMap = {};
		const hostMap = {};
	...
	domainMapping(site) {
		const map = {};
		const version = site.data.server || this.app.version;
		const upstream = this.app.upstreams[version];
	...
	initTenant(req) {
		...
		if (tenant && this.suffixes.includes(domain) && tenant in this.app.database.url) {
	...
	error(site, err) {
		try {
			if (!site.url) {
				console.warn("app.domains.error(site) missing site.url");
			}
			const host = this.hostById[site.id];
			if (!host) {
