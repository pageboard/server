...
exports.content = function (block) {
	block.type = 'block_binding';
	// "fill":"$item.content.extra\nhtml","name":"extra","filter":"element-image\nslice:0:2"
	const { filter } = block.data;
	let { fill } = block.data;
	fill = fill.split('\n').filter(x => x != "html").join('\n');
	if (filter) {
		if (filter.includes('element-image')) fill += `\nall:element-image%3Anth-of-type(-n+2)`;
		else console.warn("Unknown filter, ignored", block);
	}
	block.data.fill = fill;
	delete block.data.name;
	delete block.data.filter;
};

exports.query_form = function ({ data }) {
	if (data.redirection?.url == "") delete data.redirection.url;
};

exports.include = function (block) {
	// TODO: put a placeholder, will be something like fetch ???
};
